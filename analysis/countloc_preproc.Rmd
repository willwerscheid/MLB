...
# Calculate the wOBA values for each PA outcome.
...
sz_bot <- mean(all_pitches$sz_bot)
sz_top <- mean(all_pitches$sz_top)
sz_left <- -8.5 / 12
sz_right <- 8.5 / 12
...
ggplot(all_pitches %>% 
         ...
         summarize(is.kept = min(n) >= 1000, n = sum(n)), 
       aes(plate_x, plate_z, fill = is.kept, alpha = n)) +
  geom_tile() + 
  xlim(c(-5, 5)) +
  ylim(c(-3, 7)) +
  theme_minimal() +
  ...
  geom_segment(aes(x = sz_left, xend = sz_right, y = 0, yend = 0))
